 ...
 module.exports = ((_) => {
	...
	const GUI = {
		...
		newButton: (text, filled = true) => {
			...
			button.className =
				"button-f2h6uQ colorBrand-I6CyqQ sizeSmall-wU2dO- grow-2sR_-F";
		...
		: (([Plugin, BDFDB]) => {
				return class Spotify_Lyrics_Status extends Plugin {
							...
							BDFDB.LibraryRequires.request(
								...
								(error, response, result) => {
									...
									if (
										...
										BDFDB.LibraryModules.SpotifyUtils.getAccessToken(
											socket.accountId
										).then((promiseResult) => {
											let newSocketDevice =
												BDFDB.LibraryModules.SpotifyTrackUtils.getActiveSocketAndDevice();
									...
									try {
										...
										let songNameFormated =
											requestResult.item.name;
										let artistNameFormated =
											requestResult.item.album.artists[0]
												.name;
										let artistNameAndSongFormated =
											encodeURIComponent(
										...
										let newLyrics =
											currentLyrics[currentPositionLyrics]
												.lyrics;
					...
					getSettingsPanel() {
						...
						let exampleLyrics = GUI.newLabel(
						...
						let saveButton = GUI.setSuggested(
							GUI.newButton("Save", true)
						);
						saveButton.title = "Save the current state";
						saveButton.onclick = () => {
							try {
								sEmojiBox.value.length > 1
									? (sEmojiBox.value =
											sEmojiBox.value.substring(0, 2))
									: (sEmojiBox.value = sEmojiBox.value);
								eEmojiBox.value.length > 1
									? (eEmojiBox.value =
											eEmojiBox.value.substring(0, 2))
									: (eEmojiBox.value = eEmojiBox.value);
								...
								BdApi.showToast("Settings were saved!", {
									type: "success",
								});
								noLyricsYet = false;
							} catch (error) {
								BdApi.showToast("Error while saving!", {
									type: "error",
								});
							}
							// Restart
						...
						let emojiLink = GUI.newHyperlink(
							"Ctrl+click to open :",
							"Emoji list",
							"https://emojiterra.com/",
							"List of compatible emojis"
						);
